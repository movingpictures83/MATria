...
from networkx_mod.utils import open_file
	...
	try:
        ...
        if not multigraph:
            if not G.has_edge(source, target):
                G.add_edge(source, target, edge, weight=weight)
            else:
            ...
            G.add_edge(source, target, key, edge, weight=weight)
...
def literal_stringizer(value):
    ...
    ``networkx_mod.readwrite.gml.literal_destringizer`` function.
    """
    def stringize(value):
