...
#        nodes=v
    ...
    for n in G.nbunch_iter(v):
        if sp is None:
            length=networkx_mod.single_source_shortest_path_length(G,n)
            L = len(length)
        else:
            try:
                length=sp[n]
                ...
                raise networkx_mod.NetworkXError('Format of "sp" is invalid.')
        if L != order:
            msg = "Graph not connected: infinite path length"
            raise networkx_mod.NetworkXError(msg)
