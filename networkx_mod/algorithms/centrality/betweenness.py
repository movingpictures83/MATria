...
def betweenness_centrality(G, k=None, normalized=True, weight=None, mynodes=None,
                           endpoints=False,
                           seed=None):
    ...
    for s in nodes:
        ...
        if weight is None:  # use BFS
           ...
           if (mynodes is None):
            ...
            S, P, sigma = _single_source_dijkstra_path_product(G, s, weight)
        # accumulation
        if endpoints:
...
def _single_source_dijkstra_path_basic(G, s, weight='weight'):
    ...
    while Q:
        ...
        for w, edgedata in G[v].items():
            vw_dist = dist - math.log(abs(edgedata.get(weight, 1)))
...
def _single_source_dijkstra_path_product(G, s, weight='weight'):
    # modified from Eppstein
    S = []
    P = {}
    for v in G:
    ...
    while Q:
        ...
        for w, edgedata in G[v].items():
            ...
            if w not in D and (w not in seen or (w[len(w)-1] == '+' and vw_dist > seen[w]) or (w[len(w)-1] == '-' and vw_dist < seen[w])):
                seen[w] = vw_dist
                push(Q, (abs(vw_dist), next(c), v, w))
                sigma[w] = 0.0
                P[w] = [v]
            elif vw_dist == seen[w]:  # handle equal paths
                sigma[w] += sigma[v]
