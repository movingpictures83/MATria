...
class TestEigenvectorCentrality(object):
    ...
    def test_K5(self):
        ...
        b=networkx_mod.eigenvector_centrality(G)
        v=math.sqrt(1/5.0)
        b_answer=dict.fromkeys(G,v)
        for n in sorted(G):
            assert_almost_equal(b[n],b_answer[n])
        nstart = dict([(n,1) for n in G])
        b=networkx_mod.eigenvector_centrality(G,nstart=nstart)
        for n in sorted(G):
            assert_almost_equal(b[n],b_answer[n])
    ...
    def test_P3_unweighted(self):
        ...
        G=networkx_mod.path_graph(3)
        b_answer={0: 0.5, 1: 0.7071, 2: 0.5}
        b=networkx_mod.eigenvector_centrality_numpy(G, weight=None)
        for n in sorted(G):
            assert_almost_equal(b[n],b_answer[n],places=4)
    ...
    def test_maxiter(self):
        G=networkx_mod.path_graph(3)
        b=networkx_mod.eigenvector_centrality(G,max_iter=0)
...
class TestEigenvectorCentralityExceptions(object):
    ...
    def test_multigraph(self):
        e = networkx_mod.eigenvector_centrality(networkx_mod.MultiGraph())

    @raises(networkx_mod.NetworkXException)
    def test_multigraph_numpy(self):
        e = networkx_mod.eigenvector_centrality_numpy(networkx_mod.MultiGraph())


    @raises(networkx_mod.NetworkXException)
    def test_empty(self):
        e = networkx_mod.eigenvector_centrality(networkx_mod.Graph())

    @raises(networkx_mod.NetworkXException)
    def test_empty_numpy(self):
        e = networkx_mod.eigenvector_centrality_numpy(networkx_mod.Graph())
