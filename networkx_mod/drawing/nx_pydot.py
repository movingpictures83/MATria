...
#    BSD license.
import importlib
from networkx_mod.utils import open_file, make_str
...
def pydot_from_networkx_mod(N):
    """Create a Pydot graph from a NetworkX graph."""
    from warnings import warn
    warn('pydot_from_networkx_mod is replaced by to_pydot', DeprecationWarning)
    return to_pydot(N)

def networkx_mod_from_pydot(D, create_using=None):
    """Create a NetworkX graph from a Pydot graph."""
    from warnings import warn
    warn('networkx_mod_from_pydot is replaced by from_pydot',
         DeprecationWarning)
    return from_pydot(D)
...
def pydot_layout(G,prog='neato',root=None, **kwds):
    ...
    if D=="":  # no data returned
        ...
        print("P=pydot_from_networkx_mod(G)")
